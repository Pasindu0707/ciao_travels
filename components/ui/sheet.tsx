...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-background shadow-elev transition ease-luxury data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-500 data-[state=open]:duration-700",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <DialogPrimitive.Content
      ...
      {...props}
    >
      <DialogPrimitive.Close className="focus-ring absolute right-6 top-6 z-10 rounded-full border border-border p-2 text-foreground opacity-80 transition-all duration-500 ease-luxury hover:border-gold hover:text-gold hover:opacity-100">
        <X className="h-4 w-4" />
